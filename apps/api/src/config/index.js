...
class AppConfigClass {
  ...
  get MONGO_OPTIONS() {
    const isLocalMongo =
      this.MONGO_URI.includes("localhost") ||
      this.MONGO_URI.includes("127.0.0.1") ||
      this.MONGO_URI.includes("@mongodb:");
  ...
  rateLimitLogger(req, res, next) {
    ...
    res.send = function (data) {
      if (res.statusCode === 429) {
        console.warn(
          `[RATE_LIMIT] ${new Date().toISOString()} - IP: ${req.ip} - ${
            req.method
          } ${req.path} - User-Agent: ${req.get("User-Agent")}`
        );
      }
